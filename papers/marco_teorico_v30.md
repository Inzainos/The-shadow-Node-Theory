...
Un segundo caso de evidencia biologica en el eje ACO-A proviene del analisis TCGA sobre 2,746 pacientes oncologicos (BRCA, LUAD, GBM, COAD), en el que la red de senalizacion genica intra-celular exhibe la misma estructura de colapso orbital acoplado que la dinamica epidemiologica de Omicron. En este dominio, la friccion biologica F_bio —definida como el promedio TPM de los diez genes guardianes canonicos dividido por 100— opera como predictor del exponente de absorcion Delta: cohortes con F_bio elevado presentan Delta reducido, correspondiente a una extraccion post-colapso mas contenida, mientras que la saturacion de F_bio —evidenciada por la sobreexpresion del propio sensor, ATM_UP— indica que el sistema de friccion ha alcanzado su limite operativo y la red se aproxima al regimen de extraccion desregulada. ATM_UP como hub universal funciona de modo estrictamente analogo a la friccion epidemiologica en Omicron: es el detector sistemico del estres que, al convertirse el mismo en la anomalia, senala que la amortiguacion esta saturada. El vinculo con la Ley de Inevitabilidad del Colapso es directo: cuando cinco o mas nodos hub colapsan simultaneamente —el 5-Event Wall, documentado en el 2-5% del corpus TCGA—, la funcion de riesgo h(tau) alcanza su maximo localizable empiricamente, consistente con h(tau)>0 para todo tau finito bajo el regimen ACO-A. La convergencia de estos dos dominios biologicos independientes —epidemiologia viral y oncologia molecular— sobre la misma estructura parametrica del eje ACO-A constituye evidencia multi-escala de la generalidad del marco SNT.
...
- **PLOS Complex Systems (PCSY-D-26-00059):** REVISIÓN MAYOR — deadline 10 ago
  2026. Cifras actualizadas a v30 (721 casos). Editor: Haroldo V. Ribeiro;
  EIC: Hocine Cherifi.
...
## Modulo XIII.6: Dominio G — Satelizacion Molecular: Red de Senalizacion Genica (TCGA, n=2,746)

La satelizacion no es un fenomeno exclusivo de sistemas ecologicos o astronomicos: opera igualmente en la red de senalizacion intra-celular, donde genes hub de alta conectividad extraen capacidad regulatoria de genes satelite perifericos siguiendo la misma logica de potencia que rige la competencia interespecifica o la acrecion gravitacional. El Dominio G incorpora al corpus SNT un nuevo nivel de resolucion ontologica: la escala sub-celular. En este contexto, la satelizacion molecular se define como el proceso por el cual un gen hub —caracterizado por alta centralidad en la red de proteina-proteina y por su papel de nodo integrador de senales— impone perturbaciones correlacionadas en genes satelite funcionalmente dependientes, de modo que la anomalia del hub arrastra co-anomalias en la periferia de la red. La evidencia empirica proviene del analisis sistematico del consorcio TCGA sobre un corpus de 2,746 pacientes distribuidos en cuatro cohortes oncologicas: cancer de mama (BRCA, n=1,231), adenocarcinoma pulmonar (LUAD, n=600), glioblastoma multiforme (GBM, n=391) y cancer colorrectal (COAD, n=524). La metodologia adopto umbral de anomalia |Z|>2.5 respecto al baseline de cada cohorte, identificando sobreexpresion o subexpresion significativa en el panel de genes hub SNT. Aproximadamente el 31% de los pacientes presenta al menos una anomalia en genes hub, y entre el 2% y el 5% exhibe cinco o mas eventos simultaneos, definiendo el denominado 5-Event Wall.

El analisis por cohorte revela patrones de co-ocurrencia que, leidos en clave SNT, identifican con precision que gen opera como hub satelizador y que genes constituyen la periferia satelizada. En BRCA, los pares de mayor frecuencia —AURKB_UP + PLK1_UP (15 pacientes) y BUB1_UP + PLK1_UP (15 pacientes)— senalan al checkpoint mitotico como subsistema satelizado: PLK1, quinasa maestra del huso, actua como hub que arrastra co-activacion de AURKB y BUB1, generando un estado de hiperactividad mitotica correlacionada que la SNT interpreta como satelizacion cinetica. En LUAD, ATM emerge como hub sensor universal: los pares ATM_UP + BRAF_UP, ATM_UP + BRCA2_UP y ATM_UP + PIK3CA_UP (11 pacientes cada uno) indican que la sobreexpresion del sensor de dano DNA co-ocurre sistematicamente con la activacion de ejes oncogenicos (BRAF, PIK3CA) y de reparacion compensatoria (BRCA2). La interpretacion SNT es directa: ATM no solo detecta el estres replicativo sino que, al hacerlo, activa como hub una constelacion de nodos satelite cuya respuesta colectiva define la trayectoria del colapso. En GBM, el patron BRAF_UP + BRCA2_UP (8 pacientes) y PLK1_UP + RAD51_UP (6 pacientes) refleja un estado dual de inestabilidad mitotica y colapso de reparacion de DNA, coherente con la arquitectura de alta agresividad biologica del glioblastoma. En COAD, APC se consolida como hub del eje WNT: los tripletes APC_UP + BRAF_UP, APC_UP + KRAS_UP y APC_UP + PTEN_UP (9 pacientes cada uno) demuestran que la anomalia del guardian canonico de la via WNT arrastra co-eventos en los ejes RAS-MAPK y PI3K, configurando la satelizacion mas coordinada del corpus.

Los candidatos al 5-Event Wall constituyen la manifestacion mas extrema del proceso de satelizacion molecular: en ellos, cinco o mas nodos hub colapsan de forma simultanea, superando la capacidad de amortiguacion de la red. El candidato mas solido corresponde a LUAD, con dos combinaciones de 9 pacientes cada una (1.5% de la cohorte): ATM_UP | BRAF_UP | BRCA2_UP | PIK3CA_UP | SMAD4_UP y ATM_UP | BRAF_UP | BRCA2_UP | PTEN_UP | SMAD4_UP. La estructura de estos quintetos es paradigmatica en terminos SNT: el sensor de dano (ATM) colapsa junto al driver oncogenico primario (BRAF), el eje de senalizacion proliferativa (PIK3CA), el supresor tumoral dual (PTEN) y el guardian del eje TGF-beta (SMAD4), configurando un colapso multi-dominio en el que ningun subsistema de la red conserva capacidad regulatoria independiente. En COAD, el quinteto APC_UP | ATM_UP | KRAS_UP | PIK3CA_UP | PTEN_UP (8 pacientes, 1.5%) reproduce la misma logica: colapso simultaneo de WNT (APC), RAS-MAPK (KRAS), PI3K (PIK3CA, PTEN) y sensor DNA (ATM). Los casos de BRCA (BRCA2_UP | BUB1_UP | FANCD2_UP | PLK1_UP | RAD51_UP, 4 pacientes) y GBM (BRCA1_UP | BUB1_UP | CHEK2_UP | E2F1_UP | TOP2A_UP, 2 pacientes) presentan frecuencias menores, consistentes con la mayor heterogeneidad tumoral de esas cohortes.

ATM_UP como hub universal merece designacion explicita dentro del Dominio G. Su presencia en 12 de 29 combinaciones multi-evento en LUAD y en 8 de 18 en COAD establece un patron analogo al del nodo orquestador en redes ecologicas y astronomicas: un elemento cuya anomalia no es epifenomeno sino condicion permisiva del colapso sistemico. En escala molecular, ATM es el detector de ruptura de doble cadena de DNA y activador del checkpoint G2/M; cuando ATM_UP no refleja activacion adaptativa sino saturacion del sistema de respuesta al dano, la red pierde su regulador maestro y los nodos satelite quedan expuestos a la dinamica de extraccion desregulada, exactamente como predice la SNT para cualquier dominio en que el hub colapsa antes que la periferia.

La diferencia cualitativa entre la satelizacion molecular (Dominio G) y la satelizacion ecologica (Dominios E y F del Modulo XIII) reside en tres dimensiones: escala temporal (microsegundos a horas frente a generaciones), mecanismo de acoplamiento (interaccion proteina-proteina y fosforilacion en cascada frente a competencia por recursos fisicos), y naturaleza de la friccion. En el Dominio G, la friccion biologica F_bio se operacionaliza como el promedio TPM de los genes guardianes (TP53, BRCA1, BRCA2, MLH1, ATM, CHEK2, RAD51, FANCD2, RB1, PTEN) dividido por 100: una metrica de integridad del sistema de control de dano, analoga funcional de la friccion institucional F en el corpus original. La relacion entre F_bio y el exponente de absorcion post-colapso Delta —donde Delta cuantifica la velocidad de extraccion del hub sobre la periferia tras el colapso orbital acoplado— extiende el eje ACO-A al nivel sub-celular. El Dominio G incorpora asi el nivel mas granular del corpus SNT, cerrando el espectro empirico desde la escala galactica (Modulo XIV) hasta la red genica intra-celular, y refuerza la hipotesis central del marco: la ley de potencia de satelizacion es una propiedad emergente de la conectividad diferencial, independiente del sustrato fisico del sistema.
